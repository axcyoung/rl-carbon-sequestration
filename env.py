...
def dqn2(parameters, max_episode_num = 10, steps_per_episode=100):
    ...
    with U.make_session(num_cpu=32):
        ...
        # action =  if np.random.uniform(0,1) > exploration  else env.action_space.sample()
        ...
        for episode in range(max_episode_num): # each episode: restart run, keep learning on old weights
            ...
            for step in range(steps_per_episode): # each step = 1 year, each episode = steps_per_episode years
                ...
                if step%25==0: print(f"   [{episode}-{step}] preaction: state={env.state}")
                # Pick action and update exploration to the newest value
            ...
            logger.record_tabular("episodes", len(episode_rewards))
            logger.record_tabular("mean episode reward", round(np.mean(episode_rewards[-101:-1]), 1))
            logger.record_tabular("% time spent exploring", int(100 * exploration.value(total_stepct)))
            logger.dump_tabular()
            
            print(f"### episode {episode} ###: episode reward={episode_rewards[-1]} | State={env.state} | Total carbon={env.total_carbon()}\n")

    
        print(f"\n Actions taken: {actions_taken}; number of actions_taken={len(actions_taken)}")
        act_fp = f"output/{time.time()}_act.pkl"
        episode_rewards = np.array(episode_rewards)/steps_per_episode
        print(f"\n Ending: reward={episode_rewards}\n ending state = {env.state}\n Act filepath = {act_fp}")
...
if __name__ == '__main__':
    ...
    dqn2(parameters, max_episode_num = 20, steps_per_episode=100)
    print("\n", parameters)
